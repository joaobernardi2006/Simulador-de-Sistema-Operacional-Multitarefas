...
/* ============================================================================
 * atualizar_prioridades_dinamicas — envelhece as tarefas da fila de prontas.
 *
 * Algoritmo do livro do Prof. Maziero (escalonamento por prioridades com
 * envelhecimento, seção 5.5.2 / "task aging"):
 *   - a cada turno do escalonador, as tarefas que aguardam na fila de prontas
 *     têm sua prioridade dinâmica AUMENTADA de alpha:  pd <- pd + alpha;
 *   - quando uma tarefa é ESCOLHIDA para receber o processador, sua prioridade
 *     dinâmica é igualada à estática: pd <- pe (feito no despacho, em
 *     simulacao.c, e ao retornar de suspensão).
 *
 * Assim, alpha é um INCREMENTO por turno de espera (alpha maior = envelhece
 * mais rápido), e o envelhecimento zera sempre que a tarefa ganha a CPU,
 * garantindo a proporcionalidade descrita no livro e evitando inanição.
 * ============================================================================ */
void atualizar_prioridades_dinamicas(Queue *fila_prontas, int alpha)
{
    if (fila_vazia(fila_prontas) || alpha <= 0)
        return;

    for (int i = fila_prontas->front; i != fila_prontas->back;
         i = (i + 1) % fila_prontas->size)
    {
        TCB *t = fila_prontas->array[i];
        if (t->estado != PRONTA)
            continue;
        t->prioridade_dinamica += alpha; /* pd <- pd + alpha (Maziero) */
    }
}

/* ============================================================================
 * escalonador_PRIOPENV — Prioridade Preemptiva com Envelhecimento
 *
 * Seleciona a tarefa com maior prioridade_dinamica (envelhecida).
 * Em caso de empate de prioridade_dinamica, aplica os critérios em ordem:
 *   1. Maior prioridade estática.
 *   2. Tarefa já em execução (evita troca de contexto desnecessária).
 *   3. Menor instante de ingresso.
 *   4. Menor duração total.
 *   5. Sorteio (marca diagrama com sorteio = 1).
 * ============================================================================ */
                          ...
                          DiagramaGantt **diagrama, int tick)
{
    ...
    TCB *melhor = tarefa_atual; /* Candidata com maior prioridade_dinamica até agora */

    for (int i = fila_prontas->front; i != fila_prontas->back;
         i = (i + 1) % fila_prontas->size)
    {
        TCB *candidata = fila_prontas->array[i];

        if (candidata->estado != PRONTA)
            continue;
        ...
        /* Candidata com prioridade_dinamica MAIOR vence claramente */
        if (candidata->prioridade_dinamica > melhor->prioridade_dinamica)
        {
            melhor = candidata;
        }
        /* Empate de prioridade_dinamica — aplica critérios de desempate */
        else if (candidata->prioridade_dinamica == melhor->prioridade_dinamica)
        {
            /* Critério 1: maior prioridade estática */
            if (candidata->prioridade > melhor->prioridade)
            {
                melhor = candidata;
                continue;
            }
            ...
            /* Critério 2: prefere a tarefa já em execução */
            if (candidata == tarefa_atual) { melhor = candidata; continue; }
            if (melhor    == tarefa_atual) { continue; }

            /* Critério 3: menor instante de ingresso */
            if (candidata->ingresso < melhor->ingresso)
                melhor = candidata;
            else if (candidata->ingresso > melhor->ingresso)
                continue;

            /* Critério 4: menor duração total */
            else if (candidata->duracao < melhor->duracao)
                melhor = candidata;
            else if (candidata->duracao > melhor->duracao)
                continue;

            /* Critério 5: sorteio — marca ambas as células no diagrama */
            else
            {
                if (tick_valido(tick))
                {
