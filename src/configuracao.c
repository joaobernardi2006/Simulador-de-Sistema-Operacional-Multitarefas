...
void parsear_acoes(TCB *t, const char *txt)
{
    ...
    while (tok != NULL)
    {
        int mut = -1, inst = -1, dur = 0, tipo = ACAO_NENHUMA;

        if      (sscanf(tok, "ML%d:%d", &mut, &inst) == 2) tipo = ACAO_ML;
        else if (sscanf(tok, "MU%d:%d", &mut, &inst) == 2) tipo = ACAO_MU;
        else if (sscanf(tok, "ml%d:%d", &mut, &inst) == 2) tipo = ACAO_ML;
        else if (sscanf(tok, "mu%d:%d", &mut, &inst) == 2) tipo = ACAO_MU;
        /* E/S: IO:xx-yy  (xx = instante, yy = duracao) */
        else if (sscanf(tok, "IO:%d-%d", &inst, &dur) == 2) tipo = ACAO_IO;
        else if (sscanf(tok, "io:%d-%d", &inst, &dur) == 2) tipo = ACAO_IO;
